<div align="center">

# Hi, I’m Raul, a content creator for journals (PhD student)

**Open-source research software for neural interfaces**

[![EMG](https://img.shields.io/badge/EMG-%22logical%22%20squiggly%20lines-blue?style=flat-square)](#)
[![Myocontrol](https://img.shields.io/badge/myocontrol-think%20it%20(hopefully)%20move%20it-green?style=flat-square)](#)
[![Simulations](https://img.shields.io/badge/simulations-trust%20me%20bro-purple?style=flat-square)](#)
[![Neurorehab](https://img.shields.io/badge/Neurorehab-assisting%20function-orange?style=flat-square)](#)

[![Buy Me a Coffee](https://img.shields.io/badge/Buy%20me%20a%20coffee-support-yellow?style=for-the-badge&logo=buymeacoffee)](https://buymeacoffee.com/raulsimpetru)

</div>

---

<div align="center">

<table width="90%">
<tr>
<td width="50%" valign="top" align="center">

## [MyoGestic](https://github.com/NsquaredLab/MyoGestic)

**Real-time myocontrol**

Visualization · recording · closed-loop control

</td>
<td width="50%" valign="top" align="center">

## [MyoGen](https://github.com/NsquaredLab/MyoGen)

**Electrophysiological simulation framework**

EMG · MU pools · force · benchmarking

</td>
</tr>
</table>

</div>

---

<div align="center">

**Myo** means muscle, which is where many of these tools begin: MyoGen, MyoGestic, and MyoVerse.

But there is also a hidden **my** in there. I like to think that once you clone the code, adapt it, and build with it, the tool becomes yours too.

</div>
